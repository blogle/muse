...
Completed the Wave 1 tooling work, including the pinned frontend environment
repair: Nix shell, shared-artifact Crane checks, Cargo development/CI/release/Coz
profiles, quality/benchmark commands, Coz entry point, and deterministic
Playwright browser and font configuration.
...
- `flake.nix` — added a shared-`cargoArtifacts` workspace check derivation;
  exports the pinned Playwright browsers path, disables browser downloads, and
  supplies deterministic DejaVu-based fontconfig.
...
- Playwright smoke package install and Chromium browser test from a temporary
  package outside the repository.
- `nix develop --command bash -c 'printf "PLAYWRIGHT_BROWSERS_PATH=%s\\nPLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=%s\\nFONTCONFIG_FILE=%s\\n" "$PLAYWRIGHT_BROWSERS_PATH" "$PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD" "$FONTCONFIG_FILE"; test -d "$PLAYWRIGHT_BROWSERS_PATH"; test -f "$FONTCONFIG_FILE"; fc-match sans-serif; playwright --version'`
- `nix develop --command playwright screenshot --browser chromium about:blank /tmp/opencode/playwright-fresh-shell.png`
- `nix develop --command pnpm --dir /tmp/opencode/muse-playwright-smoke install`
- `nix develop --command pnpm --dir /tmp/opencode/muse-playwright-smoke test`

The temporary `/tmp/opencode` packages were disposable acceptance fixtures and
...
- `scripts/check.sh`: passed all five Cargo quality commands, `nix flake check`,
  and `git diff --check`.
- Fresh shell frontend environment: `PLAYWRIGHT_BROWSERS_PATH` resolves to the
  pinned `pkgs.playwright-driver.browsers` output; `FONTCONFIG_FILE` resolves to
  the config generated by `pkgs.makeFontsConf` using pinned minimal DejaVu fonts
  with host font directories/includes disabled; browser download is disabled.
  `fontconfig` is explicitly in the shell. Nix provides `playwright-driver`
  1.63.0 and `playwright-test` 1.63.0; the CLI reports `Version 1.63.0`.
- External Playwright smoke: a temporary `@playwright/test` package at version
  1.63.0 installed with browser downloads disabled, then launched Chromium from
  `PLAYWRIGHT_BROWSERS_PATH` and passed a minimal browser test without a custom
  executable path or runtime `/nix/store` scan. The fresh-shell Nix CLI
  screenshot command also successfully launched Chromium.
...
- [x] Node, pnpm, Playwright test and browser packages are present in the dev
  shell.
- [x] Playwright browser/fontconfig environment is pinned and usable by the
  shell's Playwright version (1.63.0).
