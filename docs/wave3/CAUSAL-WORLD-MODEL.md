...
Boundary normal relative motion is evaluated only on cross-label edges: use the canonical source tangent plane as reference, parallel-transport target velocity to source, take relative approach velocity `v_source - transport(v_target)`, and dot with the canonical source-to-target edge tangent at source. Positive means approach/convergence; negative means separation/divergence. For example, source motion toward target and target motion toward source both produce a positive contribution. Negating both velocities negates the result. Signed tangential interaction uses the same `v_source - transport(v_target)` relative vector dotted with the source-oriented edge-perpendicular `normalize(p_source cross edge_tangent)`; it reverses under velocity negation. Unsigned shear is its absolute magnitude and is invariant under velocity negation.
